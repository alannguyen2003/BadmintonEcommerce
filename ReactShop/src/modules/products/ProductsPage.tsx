...
export function ProductsPage() {
  ...
  const onUploadImages = (files: FileList | null) => {
    ...
    Promise.all(tasks).then((newItems) => {
      setImages((prev) => {
        ...
        if (!merged.some((x) => x.isPrimary) && merged[0])
  ...
  const addOptionValue = (optionId: string, value = "") => {
    setOptions((prev) =>
      prev.map((o) =>
        o.id === optionId ? { ...o, values: [...o.values, value] } : o,
      ),
    );
  };
  ...
  const submitProduct = async () => {
    ...
    const normalizedImages = images.map((img, idx) => ({
      ...img,
      isPrimary: idx === images.findIndex((i) => i.isPrimary),
    }));
    await createProduct({
      name: name.trim(),
      categoryId,
    ...
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400">Product table and modal CRUD</p>
        </div>
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="rounded border border-slate-700 p-3">
            <div className="mb-2 flex items-center justify-between">
              <p className="text-xs text-slate-400">Product Images</p>
            ...
            <div className="grid grid-cols-5 gap-2">
              {images.length === 0 ? (
                <p className="col-span-5 text-center text-xs text-slate-400">
                  Khong co ban ghi nao hien co
                </p>
              ) : (
                images.map((img) => (
                  <div
                    key={img.id}
                    className={`overflow-hidden rounded border ${img.isPrimary ? "border-cyan-400" : "border-slate-700"}`}
                  >
                    <img
                      src={img.dataUrl}
                      alt={img.name}
                    ...
                    <div className="flex items-center justify-between p-1">
                      <button
                        className={`rounded px-2 py-0.5 text-xs ${img.isPrimary ? "bg-cyan-700" : "bg-slate-700"}`}
                        onClick={() =>
                          setImages((prev) =>
                            prev.map((x) => ({
                              ...x,
                      ...
                      <button
                        className="rounded bg-rose-600 px-2 py-0.5 text-xs"
                        onClick={() =>
                          setImages((prev) =>
              ...
              {options.map((option) => (
                ...
                >
                  ...
                  <input
                    ...
                    className="mb-2 w-full rounded border border-slate-600 bg-slate-900 px-2 py-1"
                  />
                  <label className="mb-1 block text-xs text-slate-400">
                    Option Values
                  </label>
                    ...
                    variants.map((row, idx) => (
                      ...
                      >
                        {options
                          .filter((o) => o.name.trim() && o.values.length > 0)
                          .map((o) => (
                            <td key={o.id} className="py-1">
                              <select
                                value={row.optionValues[o.id] || ""}
                                onChange={(e) =>
                                  updateSkuOptionValue(
                                    idx,
                                    o.id,
                                    e.target.value,
                                  )
                                }
                                className="w-full rounded border border-slate-700 bg-slate-900 px-1 text-center text-xs"
                              >
                        ...
                        <td>
                          <input
                            ...
                            className="mx-auto w-24 rounded border border-slate-700 bg-slate-900 px-1 text-center"
                          />
                          <p className="text-[10px] text-slate-500">
                            {formatInteger(row.price)}
                          </p>
                        </td>
                        <td>
                          <input
                            ...
                            className="mx-auto w-20 rounded border border-slate-700 bg-slate-900 px-1 text-center"
                          />
                          <p className="text-[10px] text-slate-500">
      ...
      >
        <div className="space-y-2">
          <p className="text-sm text-slate-300">Confirm delete?</p>
          <button
